...
export async function handleGroupParticipantsUpdate(sock, update, userId) {
const { id, participants, action, author } = update;
console.log(`[DEBUG] Group event: ${action} in ${id}`);

const currentData = JSON.parse(fs.readFileSync('./data/bot_data.json', 'utf8'));
...
export async function handleStubGroupEvent(sock, msg, from) {
